...
function ArtistRow({ reverse, speed, badgeColor, onFollow }) {
  const rowRef = useRef(null);
  const animationRef = useRef();
  const rowArtists = [...artists, ...artists];

  // Smooth JS-based animation
  useEffect(() => {
    ...
    function step(ts) {
      ...
      pos = ((elapsed / duration) * totalWidth * direction) % totalWidth;
      row.style.transform = `translateX(${reverse ? pos : -pos}px)`;
      reqId = requestAnimationFrame(step);
    }
    ...
    row.parentElement.addEventListener("mouseenter", pause);
    row.parentElement.addEventListener("mouseleave", resume);

    return () => {
      cancelAnimationFrame(reqId);
      row.parentElement.removeEventListener("mouseenter", pause);
      row.parentElement.removeEventListener("mouseleave", resume);
    };
  }, [reverse, speed, rowArtists.length]);
  ...
  return (
    <Box
      ...
      _before={{
        ...
        zIndex: 2,
        pointerEvents: "none",
        bgGradient: "linear(to-r, blackAlpha.800 70%, transparent 100%)",
      }}
      _after={{
        content: '""',
        position: "absolute",
        ...
        zIndex: 2,
        pointerEvents: "none",
        bgGradient: "linear(to-l, blackAlpha.800 70%, transparent 100%)",
      }}
    >
      <Flex
        ref={rowRef}
        ...
        align="center"
        minW="max-content"
        // Remove CSS animation, handled by JS now
...
export default function ArtistCarousel() {
  ...
  return (
    ...
    >
      ...
      <ArtistRow speed="18s" badgeColor="purple" onFollow={handleFollow} />
      <ArtistRow
        reverse
        speed="28s"
        badgeColor="pink"
      ...
      />
      <ArtistRow speed="22s" badgeColor="yellow" onFollow={handleFollow} />
      {modalArtist && (
        <ArtistModal
          artist={modalArtist}
          isOpen={modalOpen}
